...
static unsigned int get_zcut_for_boxes(sotl_device_t *dev)
{
  ...
  int count[2] = { 0, 0 };
  int next[2];
  unsigned int boxes[dev->domain.total_boxes];
  int index[2];

  err = clEnqueueReadBuffer (dev->queue, dev->box_buffer, CL_TRUE, 0,
			     dev->domain.total_boxes * sizeof(*boxes), 
			     &boxes, 0, NULL, NULL);
  check (err, "Failed to read box buffer");
  clFinish (dev->queue);

  index[0] = -1;
  index[1] = dev->domain.boxes[2];

  while (index[0] < index[1] - 1) {
    int bi[2];

    bi[0] = (index[0] + 1) * dev->domain.boxes[0] * dev->domain.boxes[1];
    bi[1] = (index[1] - 1) * dev->domain.boxes[0] * dev->domain.boxes[1];
    next[0] = next[1] = 0;
    for(unsigned int i = 0; i < dev->domain.boxes[0]*dev->domain.boxes[1]; i++) {
      next[0] += boxes[bi[0] + i];
      next[1] += boxes[bi[1] + i];
    }

    // look for minimal difference
    //
    if( abs(count[0]+next[0] - count[1]) < abs(count[1] + next[1] - count[0]) ) {
      count[0] += next[0];
      index[0]++;
    } else {
      count[1] += next[1];
...
void ocl_one_step_move(sotl_device_t *dev)
{
  ...
  if (force_enabled) {
	...
	if(!done) {
	  unsigned zcut = get_zcut_for_boxes(dev);
	  setZcutValues (get_global_domain()->min_ext[2],
			 get_global_domain()->min_ext[2] + zcut * BOX_SIZE,
			 get_global_domain()->max_ext[2]);
	  //done=1;
	}
      }
#endif
...
#ifdef FORCE_N_UPDATE
      // box_lennard_jones updates alternate position buffer
      	dev->cur_pb = 1 - dev->cur_pb;
#endif

#ifdef DEBUG
      {
	int *boxes = xmalloc (sizeof(int) * dev->domain.total_boxes);
	FILE* f = fopen("boxes.dump", "w");
	cl_int err;

	clFinish(dev->queue);
	err = clEnqueueReadBuffer (dev->queue, dev->box_buffer, CL_TRUE, 0,
				   sizeof(int) * dev->domain.total_boxes,
				   boxes, 0, NULL, NULL);
	check(err, "Failed to read box buffer");
	for(unsigned z = 0; z < dev->domain.boxes[2]; z++)
	  for(unsigned y = 0; y < dev->domain.boxes[1]; y++)
	    for(unsigned x = 0; x < dev->domain.boxes[0]; x++) {
	      unsigned num = z * dev->domain.boxes[0] * dev->domain.boxes[1] + y * dev->domain.boxes[0] + x;
	      fprintf (f, "box[%d,%d,%d] = %d (= n° %d)\n", z, y, x, boxes[num], num);
	    }
	fclose(f);
      }
#endif
